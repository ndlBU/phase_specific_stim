...
thetaFilter::thetaFilter(void) : DefaultGUIModel("Theta Filter", ::vars, ::num_vars)
{
	setWhatsThis(
...
thetaFilter::~thetaFilter(void)
{
}

void thetaFilter::execute(void) 
{
	systime = count * dt;
...
double thetaFilter::readFromBuffer(int index)
{
	return b[index];
}

void thetaFilter::writeToBuffer(double val)
{
	start = ((start-1) % N + N) % N;
	b[start] = val;
}

double thetaFilter::conv(double val)
{
	y = 0;
	for (int i = 0; i < N; i++)
	{
...
void thetaFilter::update(DefaultGUIModel::update_flags_t flag)
{
	switch (flag) {
	case INIT:
		wait = true;
...
void thetaFilter::initParameters()
{
	output(0) = 0;
	y = 0;
	start = 0;
	...
	file = "Filter_impulse_response.csv";
